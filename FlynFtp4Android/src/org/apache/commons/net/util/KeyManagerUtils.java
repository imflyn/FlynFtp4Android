 ...
 * </pre>
 * 
 * If using the default store type and the key password is the same as the store
 * password, these parameters can be omitted. <br/>
 * If the desired key is the first or only key in the keystore, the keyAlias
 * parameter can be omitted, in which case the code becomes:
 * 
 * <pre>
...
public final class KeyManagerUtils
{
     ...
     *            the keystore to use
     * @param keyAlias
     *            the alias of the key to use, may be {@code null} in which case
     *            the first key entry alias is used
     * @param keyPass
     ...
     * Create a client key manager which returns a particular key. Does not
     * handle server keys. Uses the default store type and assumes the key
     * password is the same as the store password. The key alias is found by
     * searching the keystore for the first private key entry
     * 
     * @param storePath
     *            the path to the keyStore
     * @param storePass

...
public final class IMAPReply
{
    ...
    /**
     * Intepret the String reply code - OK, NO, BAD - in a tagged response as a
     * integer.
     * 
     * @param line
    ...
    public static int literalCount(String line)
    {
        ...
        if (m.find())
        {
            return Integer.parseInt(m.group(1)); // Should always parse because
                                                 // we matched \d+
        }
        return -1;
    }
    ...
    /**
     * Intepret the String reply code - OK, NO, BAD - in an untagged response as
     * a integer.
     * 
     * @param line
    ...
    private static int getReplyCode(String line, Pattern pattern) throws IOException
    {
        ...
        if (m.matches())
        { // TODO would lookingAt() be more efficient? If so, then drop trailing
          // .* from patterns

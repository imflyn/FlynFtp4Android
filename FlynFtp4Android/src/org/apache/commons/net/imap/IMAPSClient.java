 ...
 * Copied from FTPSClient.java and modified to suit IMAP. If implicit mode is
 * selected (NOT the default), SSL/TLS negotiation starts right after the
 * connection has been established. In explicit mode (the default), SSL/TLS
 * negotiation starts when the user calls execTLS() and the server accepts the
 * command. Implicit usage: IMAPSClient c = new IMAPSClient(true);
 * c.connect("127.0.0.1", 993); Explicit usage: IMAPSClient c = new
 * IMAPSClient(); c.connect("127.0.0.1", 143); if (c.execTLS()) { /rest of the
 * commands here/ }
 */
public class IMAPSClient extends IMAPClient
{
    ...
    /**
     * Constructor for IMAPSClient. Sets security mode to explicit (isImplicit =
     * false).
     */
    public IMAPSClient()
    {

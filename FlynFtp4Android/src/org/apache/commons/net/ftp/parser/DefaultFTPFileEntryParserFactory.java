 ...
 * interface. This is the implementation that will be used by
 * org.apache.commons.net.ftp.FTPClient.listFiles() if no other implementation
 * has been specified.
 * 
...
public class DefaultFTPFileEntryParserFactory implements FTPFileEntryParserFactory
{
    ...
    /**
     * This default implementation of the FTPFileEntryParserFactory interface
     * works according to the following logic: First it attempts to interpret
     * the supplied key as a fully qualified classname (default package is not
     * allowed) of a class implementing the FTPFileEntryParser interface. If
     * that succeeds, a parser object of this class is instantiated and is
     * returned; otherwise it attempts to interpret the key as an identirier
     * commonly used by the FTP SYST command to identify systems.
     * <p/>
     * If <code>key</code> is not recognized as a fully qualified classname
     * known to the system, this method will then attempt to see whether it
     * <b>contains</b> a string identifying one of the known parsers. This
     * comparison is <b>case-insensitive</b>. The intent here is where possible,
     * to select as keys strings which are returned by the SYST command on the
     * systems which the corresponding parser successfully parses. This enables
     * this factory to be used in the auto-detection system.
     * <p/>
     * 
     * @param key
     *            should be a fully qualified classname corresponding to a class
     *            implementing the FTPFileEntryParser interface<br/>
     *            OR<br/>
     *            a string containing (case-insensitively) one of the following
     *            keywords:
     *            <ul>
     *            <li>{@link FTPClientConfig#SYST_UNIX UNIX}</li> <li>
     *            {@link FTPClientConfig#SYST_NT WINDOWS}</li> <li>
     *            {@link FTPClientConfig#SYST_OS2 OS/2}</li> <li>
     *            {@link FTPClientConfig#SYST_OS400 OS/400}</li> <li>
     *            {@link FTPClientConfig#SYST_AS400 AS/400}</li> <li>
     *            {@link FTPClientConfig#SYST_VMS VMS}</li> <li>
     *            {@link FTPClientConfig#SYST_MVS MVS}</li> <li>
     *            {@link FTPClientConfig#SYST_NETWARE NETWARE}</li> <li>
     *            {@link FTPClientConfig#SYST_L8 TYPE:L8}</li>
     *            </ul>
     * @return the FTPFileEntryParser corresponding to the supplied key.
     * @throws ParserInitializationException
     *             thrown if for any reason the factory cannot resolve the
     *             supplied key into an FTPFileEntryParser.
    ...
    private FTPFileEntryParser createFileEntryParser(String key, FTPClientConfig config)
    {
        ...
        { // Now try for aliases
            ...
            {
                parser = new MVSFTPEntryParser(); // Does not currently support
                                                  // config parameter
     ...
     * FTPClientConfig} parameter and creates an object implementing the
     * interface FTPFileEntryParser and uses the supplied configuration to
     * configure it.
     * </p>
     * <p>
     ...
     * 
     * @param config
     *            the config to use, may be {@code null}
     * @return the parser
     */

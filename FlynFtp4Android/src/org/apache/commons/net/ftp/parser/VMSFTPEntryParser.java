...
/**
 * Implementation FTPFileEntryParser and FTPFileListParser for VMS Systems. This
 * is a sample of VMS LIST output
 * 
 ...
 * ,
 * <P>
 * <B> Note: VMSFTPEntryParser can only be instantiated through the
 * DefaultFTPParserFactory by classname. It will not be chosen by the
 * autodetection scheme. </B>
 * <P>
 * 
...
public class VMSFTPEntryParser extends ConfigurableFTPFileEntryParserImpl
{
    ...
    private static final String DEFAULT_DATE_FORMAT = "d-MMM-yyyy HH:mm:ss";                                        // 9-NOV-2001
                                                                                                                     // 12:30:24

    /**
     * this is the regular expression used by this parser.
     */
    private static final String REGEX               = "(.*;[0-9]+)\\s*" // 1
                                                                        // file
                                                                        // and
                                                                        // version
                                                            + "(\\d+)/\\d+\\s*" // 2
                                                            ...
                                                            + "(\\S+)\\s+(\\S+)\\s+" // 3+4
                                                                                     ...
                                                                                     // and
                                                                                     // time
                                                            + "\\[(([0-9$A-Za-z_]+)|([0-9$A-Za-z_]+),([0-9$a-zA-Z_]+))\\]?\\s*" // 5(6,7,8)
                                                                                                                                // owner
                                                            + "\\([a-zA-Z]*,([a-zA-Z]*),([a-zA-Z]*),([a-zA-Z]*)\\)"; // 9,10,11
                                                                                                                     // Permissions
                                                                                                                     // (O,G,W)
     ...
     * whatever delemits one entry from the next. This parser cannot use the
     * default implementation of simply calling BufferedReader.readLine(),
     ...
     * instantiated without a {@link FTPClientConfig FTPClientConfig} parameter
     * being specified.
     * 

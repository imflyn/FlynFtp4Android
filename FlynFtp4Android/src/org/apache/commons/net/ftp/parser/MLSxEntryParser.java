 ...
 * requires that the leading space from the MLST entry is removed. MLSD entries
 * can begin with a single space if there are no facts.
 * 
 * @since 3.0
 */
public class MLSxEntryParser extends FTPFileEntryParserImpl
{
    ...
    static
    {
        ...
        TYPE_TO_INT.put("cdir", Integer.valueOf(FTPFile.DIRECTORY_TYPE)); // listed
                                                                          // directory
        TYPE_TO_INT.put("pdir", Integer.valueOf(FTPFile.DIRECTORY_TYPE)); // a
                                                                          // parent
                                                                          // dir
        TYPE_TO_INT.put("dir", Integer.valueOf(FTPFile.DIRECTORY_TYPE)); // dir
                                                                         // or
                                                                         // sub-dir
    }

    private static int                            UNIX_GROUPS[]  = { // Groups
                                                                     // in order
                                                                     // of mode
                                                                     // digits
            FTPFile.USER_ACCESS, FTPFile.GROUP_ACCESS, FTPFile.WORLD_ACCESS, };

    private static int                            UNIX_PERMS[][] = { // perm
                                                                     // bits,
                                                                     // broken
                                                                     // down by
                                                                     // octal
    ...
    public FTPFile parseFTPEntry(String entry)
    {
        ...
        for (String fact : facts)
        {
            ...
            } else if ("modify".equals(factname))
            {
                ...
                TimeZone GMT = TimeZone.getTimeZone("GMT"); // both need to be
                                                            // set for the parse
                                                            // to work OK
                sdf.setTimeZone(GMT);
            ...
            } else if (factname.startsWith("unix."))
            {
                ...
                { // e.g. 0[1]755
                    int off = factvalue.length() - 3; // only parse last 3
                                                      // digits
                    for (int i = 0; i < 3; i++)
                    {
                        int ch = factvalue.charAt(off + i) - '0';
                        if (ch >= 0 && ch <= 7)
                        { // Check it's valid octal

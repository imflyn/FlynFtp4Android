...
/***
 * FTP provides the basic the functionality necessary to implement your own FTP
 * client. It extends org.apache.commons.net.SocketClient since extending
 * TelnetClient was causing unwanted behavior (like connections that did not
 * time out properly).
 * <p>
 * To derive the full benefits of the FTP class requires some knowledge of the
 * FTP protocol defined in RFC 959. However, there is no reason why you should
 * have to use the FTP class. The {@link org.apache.commons.net.ftp.FTPClient}
 * class, derived from FTP, implements all the functionality required of an FTP
 * client. The FTP class is made public to provide access to various FTP
 * constants and to make it easier for adventurous programmers (or those with
 * special needs) to interact with the FTP protocol and implement their own
 * clients. A set of methods with names corresponding to the FTP command names
 * are provided to facilitate this interaction.
 * <p>
 ...
 * (usually 900 seconds). The FTP class will detect a premature FTP server
 ...
 * <code>FTPConectionClosedException</code> is a subclass of
 ...
 * clean up the system resources used by FTP. Before disconnecting, you may
 ...
 * getReplyStrings}. You may avoid server disconnections while the client is
 * idle by periodicaly sending NOOP commands to the server.
 * <p>
 ...
 * @version $Id: FTP.java 1448087 2013-02-20 11:20:07Z sebb $
 ***/
...
public class FTP extends SocketClient
{
    ...
    /***
     * A constant used to indicate the file(s) being transfered should be
     * treated as ASCII. This is the default file type. All constants ending in
    ...
    /***
     * A constant used to indicate the file(s) being transfered should be
     * treated as EBCDIC. Note however that there are several different EBCDIC
     * formats. All constants ending in <code>FILE_TYPE</code> are used to
     * indicate file types.
     ***/
    ...
    /***
     * A constant used to indicate the file(s) being transfered should be
     * treated as a binary image, i.e., no translations should be performed. All
     * constants ending in <code>FILE_TYPE</code> are used to indicate file
     * types.
     ***/
    ...
    /***
     * A constant used to indicate the file(s) being transfered should be
     * treated as a local type. All constants ending in <code>FILE_TYPE</code>
     * are used to indicate file types.
     ***/
    public static final int          LOCAL_FILE_TYPE              = 3;

    /***
     * A constant used for text files to indicate a non-print text format. This
     * is the default format. All constants ending in <code>TEXT_FORMAT</code>
     * are used to indicate text formatting for text transfers (both ASCII and
     * EBCDIC).
     ***/
     ...
     * control characters. All constants ending in <code>TEXT_FORMAT</code> are
     * used to indicate text formatting for text transfers (both ASCII and
     * EBCDIC).
     ***/
     ...
     * sequence of bytes. This is the default structure. All constants ending in
    ...
    /***
     * A constant used to indicate a file is to be treated as a sequence of
     * records. All constants ending in <code>_STRUCTURE</code> are used to
    ...
    /***
     * A constant used to indicate a file is to be transfered as a stream of
     * bytes. This is the default transfer mode. All constants ending in
     * <code>TRANSFER_MODE</code> are used to indicate file transfer modes.
     ***/
    public static final int          STREAM_TRANSFER_MODE         = 10;

    /***
     * A constant used to indicate a file is to be transfered as a series of
    ...
    /***
     * A constant used to indicate a file is to be transfered as FTP compressed
     * data. All constants ending in <code>TRANSFER_MODE</code> are used to
     * indicate file transfer modes.
     ***/
    ...
    // the wire.
    /**
     * The default character encoding used for communicating over an FTP control
     * connection. The default encoding is an ASCII-compatible encoding. Some
     * FTP servers expect other encodings. You can change the encoding used by
     * an FTP instance with {@link #setControlEncoding setControlEncoding}.
     */
     ...
     * with xxx must be terminated by the same numeric code xxx See section 4.2
     * of RFC 959 for details.
     */
    protected boolean                strictMultilineParsing       = false;

    /**
     * Wraps SocketClient._input_ to facilitate the reading of text from the FTP
     * control connection. Do not access the control connection via
     * SocketClient._input_. This member starts with a null value, is
     * initialized in {@link #_connectAction_}, and set to null in
     * {@link #disconnect}.
     */
    protected BufferedReader         _controlInput_;

    /**
     * Wraps SocketClient._output_ to facilitate the writing of text to the FTP
     * control connection. Do not access the control connection via
     * SocketClient._output_. This member starts with a null value, is
     * initialized in {@link #_connectAction_}, and set to null in
     * {@link #disconnect}.
     */
     ...
     * <code>DEFAULT_PORT</code> and initializes internal data structures for
     * saving FTP reply information.
     ***/
    public FTP()
    {
        super();
    ...
    /**
     * Initiates control connections and gets initial reply. Initializes
     * {@link #_controlInput_} and {@link #_controlOutput_}.
     */
    @Override
    ...
    /**
     * Sets the character encoding used by the FTP control connection. Some FTP
     * servers require that commands be issued in a non-ASCII encoding like
     * UTF-8 so that filenames with multi-byte character representations (e.g,
     * Big 8) can be specified.
     * 
     * @param encoding
    ...
    /***
     * Closes the control connection to the FTP server and sets to null some
     * internal data so that the memory may be reclaimed by the garbage
     * collector. The reply text and code information from the last command is
     * voided so that the memory it used may be reclaimed. Also sets
     * {@link #_controlInput_} and {@link #_controlOutput_} to null.
     * <p>
     * 
     ...
     * numerical response code. After invocation, for more detailed information,
     * the actual reply text can be accessed by calling {@link #getReplyString
     * getReplyString } or {@link #getReplyStrings getReplyStrings }.
     * <p>
     * 
     * @param command
     ...
     * @return The integer value of the FTP reply code returned by the server in
     ...
     * 
     * @param command
     *            The FTPCommand constant corresponding to the FTP command to
     *            send.
     * @param args
     *            The arguments to the FTP command. If this parameter is set to
     *            null, then the command is sent with no argument.
     ...
     * 
     * @param command
     *            The FTPCmd enum corresponding to the FTP command to send.
     * @return The integer value of the FTP reply code returned by the server in
     *         response to the command.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
    ...
    /***
     * Sends an FTP command with no arguments to the server, waits for a reply
     * and returns the numerical response code. After invocation, for more
     * detailed information, the actual reply text can be accessed by calling
     ...
     * will usually only use this method after you connect to the FTP server to
     * check that the connection was successful since <code> connect </code> is
     * of type void.
     * <p>
     * 
     ...
     * After calling this method, the actual reply text can be accessed from
     * either calling {@link #getReplyString getReplyString } or
     * {@link #getReplyStrings getReplyStrings }. Only use this method if you
     * are implementing your own FTP client or if you need to fetch a secondary
    ...
    /***
     * A convenience method to send the FTP USER command to the server, receive
    ...
    /**
     * A convenience method to send the FTP PASS command to the server, receive
     * the reply, and return the reply code.
     * 
     * @param password
    ...
    /***
     * A convenience method to send the FTP ACCT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param account
    ...
    /***
     * A convenience method to send the FTP ABOR command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP CDUP command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP QUIT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP REIN command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP SMNT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param dir
    ...
    /***
     * A convenience method to send the FTP PORT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param host
     *            The host owning the port.
     * @param port
     *            The new port.
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
    ...
    /***
     * A convenience method to send the FTP EPRT command to the server, receive
     * the reply, and return the reply code.
     * 
     * Examples: <code>
     * <ul>
    ...
    /***
     * A convenience method to send the FTP PASV command to the server, receive
     * the reply, and return the reply code. Remember, it's up to you to
    ...
    /***
     * A convenience method to send the FTP EPSV command to the server, receive
     * the reply, and return the reply code. Remember, it's up to you to
     * interpret the reply string containing the host/port information.
     * <p>
     * 
    ...
    /**
     * A convenience method to send the FTP TYPE command for text files to the
     * server, receive the reply, and return the reply code.
     * 
     * @param fileType
     *            The type of the file (one of the <code>FILE_TYPE</code>
     *            constants).
    ...
    /**
     * A convenience method to send the FTP TYPE command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param fileType
    ...
    /***
     * A convenience method to send the FTP STRU command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param structure
     *            The structure of the file (one of the <code>_STRUCTURE</code>
     *            constants).
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
    ...
    /***
     * A convenience method to send the FTP MODE command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param mode
    ...
    /***
     * A convenience method to send the FTP RETR command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
    ...
    /***
     * A convenience method to send the FTP STOR command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
     ...
     * 
     * @param pathname
     *            The pathname to use for the file when stored at the remote end
     *            of the transfer.
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
    ...
    /***
     * A convenience method to send the FTP STOU command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
     ...
     * 
     * @param pathname
     *            The base pathname to use for the file when stored at the
     *            remote end of the transfer. Some FTP servers require this.
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
    ...
    /***
     * A convenience method to send the FTP APPE command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
    ...
    /***
     * A convenience method to send the FTP ALLO command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param bytes
    ...
    /**
     * A convenience method to send the FTP FEAT command to the server, receive
     * the reply, and return the reply code.
     * 
    ...
    /***
     * A convenience method to send the FTP REST command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param marker
    ...
    /**
     * A convenience method to send the FTP MFMT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
     *            The pathname for which mtime is to be changed
     * @param timeval
     *            Timestamp in <code>YYYYMMDDhhmmss</code> format
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
    ...
    /***
     * A convenience method to send the FTP RNFR command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
    ...
    /***
     * A convenience method to send the FTP RNTO command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
    ...
    /***
     * A convenience method to send the FTP DELE command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
    ...
    /***
     * A convenience method to send the FTP RMD command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
    ...
    /***
     * A convenience method to send the FTP MKD command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param pathname
    ...
    /***
     * A convenience method to send the FTP PWD command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP LIST command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
     ...
     * 
     * @param pathname
     *            The pathname to list, may be {@code null} in which case the
     *            command is sent with no parameters
     * @return The reply code received from the server.
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
    ...
    /**
     * A convenience method to send the FTP MLSD command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
     ...
     * @return The reply code received from the server, may be {@code null} in
     *         which case the command is sent with no parameters
     * @exception FTPConnectionClosedException
     *                If the FTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
     *                causing the server to send FTP reply code 421. This
    ...
    /**
     * A convenience method to send the FTP MLST command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
    ...
    /***
     * A convenience method to send the FTP NLST command to the server, receive
     * the reply, and return the reply code. Remember, it is up to you to manage
     * the data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details for you.
    ...
    /***
     * A convenience method to send the FTP SITE command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param parameters
    ...
    /***
     * A convenience method to send the FTP SYST command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP STAT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP HELP command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the FTP NOOP command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /**
     * Return whether strict multiline parsing is enabled, as per RFC 959,
     * section 4.2.
     * 

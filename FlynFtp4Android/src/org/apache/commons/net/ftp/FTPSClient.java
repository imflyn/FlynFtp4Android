 ...
 * 
 * @version $Id: FTPSClient.java 1489533 2013-06-04 17:49:00Z sebb $
 * @since 2.0
 */
public class FTPSClient extends FTPClient
{
    ...
    /**
     * Constructor for FTPSClient allowing specification of protocol and
     * security mode. If isImplicit is true, the port is set to
     * {@link #DEFAULT_FTPS_PORT} i.e. 990. The default TrustManager is set from
     * {@link TrustManagerUtils#getValidateServerCertificateTrustManager()}
     * 
     * @param protocol
     *            the protocol
    ...
    /**
     * Constructor for FTPSClient, using {@link #DEFAULT_PROTOCOL} - i.e. TLS
     * The default TrustManager is set from
     * {@link TrustManagerUtils#getValidateServerCertificateTrustManager()}
     * 
     * @param isImplicit
     *            The security mode(Implicit/Explicit).
    ...
    /**
     * Constructor for FTPSClient, using {@link #DEFAULT_PROTOCOL} - i.e. TLS
     * and isImplicit {@code false} Calls
     * {@link #FTPSClient(boolean, SSLContext)}
     * 
     * @param context
     *            A pre-configured SSL Context
     */
    ...
    /**
     * Set AUTH command use value. This processing is done before connected
     * processing.
     * 
     * @param auth
     ...
     * 
     * @throws IOException
     *             If it throw by _connectAction_.
    ...
    /**
     * SSL/TLS negotiation. Acquires an SSL socket of a control connection and
     * carries out handshake processing.
     * 
     * @throws IOException
    ...
    /**
     * Returns true if new SSL sessions may be established by this socket. When
     * the underlying {@link Socket} instance is not SSL-enabled (i.e. an
     * instance of {@link SSLSocket} with {@link SSLSocket}
     * {@link #getEnableSessionCreation()}) enabled, this returns False.
     * 
    ...
    /**
     * Returns true if the socket will require client authentication. When the
     * underlying {@link Socket} is not an {@link SSLSocket} instance, returns
     * false.
     * 
     * @return true - If the server mode socket should request that the client
     *         authenticate itself.
     */
    ...
    /**
     * Configures the socket to request client authentication, but only if such
     * a request is appropriate to the cipher suite negotiated.
     * 
     * @param isWantClientAuth
     *            The want client auth flag.
     */
    ...
    /**
     * Returns true if the socket will request client authentication. When the
     * underlying {@link Socket} is not an {@link SSLSocket} instance, returns
     * false.
     * 
     * @return true - If the server mode socket should request that the client
     ...
     * handshake. When the underlying {@link Socket} is not an {@link SSLSocket}
     ...
     * this connection. When the underlying {@link Socket} is not an
    ...
    /**
     * PBSZ command. pbsz value: 0 to (2^32)-1 decimal integer. Issues the
     * command and parses the response to return the negotiated value.
     * 
     * @param pbsz
     *            Protection Buffer Size.
     * @throws SSLException
     ...
     * 
     * @param prot
     *            Data Channel Protection Level, if {@code null}, use
     *            {@link #DEFAULT_PROT}.
     * @throws SSLException
     *             If the server reply code does not equal {@code 200}.
     * @throws IOException
     *             If an I/O error occurs while sending the command.
     */
    ...
    /**
     * Send an FTP command. A successful CCC (Clear Command Channel) command
     * causes the underlying {@link SSLSocket} instance to be assigned to a
     * plain {@link Socket}
     * 
     * @param command
     *            The FTP command.
     ...
     *            null, then the command is sent with no arguments.
     * @return corresponding to the established data connection. Null is
     *         returned if an FTP protocol error is reported at any point during
     *         the establishment and initialization of the connection.
     * @throws IOException
     ...
     * the SSL handshake happens). Called by
     * {@link #_openDataConnection_(int, String)} immediately after creating the
     ...
     * 
     * @param trustManager
     *            The TrustManager implementation to set, may be {@code null}
     ...
     * <p>
     * Calls {@code setSocketFactory(null)} and
     * {@code setServerSocketFactory(null)} to reset the factories that may have
     * been changed during the session, e.g. by {@link #execPROT(String)}
     * 
    ...
    /**
     * Send the CCC command to the server. The CCC (Clear Command Channel)
     * command causes the underlying {@link SSLSocket} instance to be assigned

 ...
 * {@link org.apache.commons.net.ftp.FTPClient FTPClient} object and also
 * subordinate objects which it uses. Any class implementing the
 * {@link org.apache.commons.net.ftp.Configurable Configurable } interface can
 * be configured by this object.
 * </p>
 * <p>
 * In particular this class was designed primarily to support configuration of
 * FTP servers which express file timestamps in formats and languages other than
 * those for the US locale, which although it is the most common is not
 * universal. Unfortunately, nothing in the FTP spec allows this to be
 * determined in an automated way, so manual configuration such as this is
 * necessary.
 * </p>
 * <p>
 * This functionality was designed to allow existing clients to work exactly as
 * before without requiring use of this component. This component should only
 * need to be explicitly invoked by the user of this package for problem cases
 * that previous implementations could not solve.
 * </p>
 * <h3>Examples of use of FTPClientConfig</h3> Use cases: You are trying to
 * access a server that
 * <ul>
 * <li>lists files with timestamps that use month names in languages other than
 * English</li>
 * <li>lists files with timestamps that use date formats other than the American
 * English "standard" <code>MM dd yyyy</code></li>
 * <li>is in different timezone and you need accurate timestamps for dependency
 * checking as in Ant</li>
 * </ul>
 * <p>
 * Unpaged (whole list) access on a UNIX server that uses French month names but
 * uses the "standard" <code>MMM d yyyy</code> date formatting
 * 
 * <pre>
 ...
 * </p>
 * <p>
 * Unpaged (whole list) access on a VMS server that uses month names in a
 * language not {@link #getSupportedLanguageCodes() supported} by the system.
...
public class FTPClientConfig
{
     ...
     * command. We set these to be a Unix-type system. This may happen if the
     * ftpd in question was compiled without system information.
     * 
     ...
     * @since 3.1
     */
    // Full string is "MACOS Peter's Server"; the substring below should be
    // enough
     ...
     * 
     * @param systemKey
     *            key representing system type of the server being connected to.
     *            See {@link #getServerSystemKey() serverSystemKey}
     */
     ...
     *            See {@link #setDefaultDateFormatStr(String)
     *            defaultDateFormatStr}
     * @param recentDateFormatStr
     *            See {@link #setRecentDateFormatStr(String)
     *            recentDateFormatStr}
     ...
     * general type of server to which the client connects. Should be either one
     * of the <code>FTPClientConfig.SYST_*</code> codes or else the fully
     * qualified class name of a parser implementing both the
     * <code>FTPFileEntryParser</code> and <code>Configurable</code> interfaces.
     * 
    ...
    /**
     * <p>
     * getter for the {@link #setServerLanguageCode(String) serverLanguageCode}
     * property.
     * </p>
     * 
    ...
    /**
     * <p>
     * getter for the {@link #setLenientFutureDates(boolean) lenientFutureDates}
     * property.
     * </p>
     * 
    ...
    /**
     * <p>
     * setter for the defaultDateFormatStr property. This property specifies the
     * main date format that will be used by a parser configured by this
     * configuration to parse file timestamps. If this is not specified, such a
     * parser will use as a default value, the most commonly used format which
     * will be in as used in <code>en_US</code> locales.
     * </p>
     * <p>
     * This should be in the format described for
     * <code>java.text.SimpleDateFormat</code>. property.
     * </p>
     * 
     * @param defaultDateFormatStr
    ...
    /**
     * <p>
     * setter for the recentDateFormatStr property. This property specifies a
     * secondary date format that will be used by a parser configured by this
     * configuration to parse file timestamps, typically those less than a year
     * old. If this is not specified, such a parser will not attempt to parse
     ...
     * {@link #setRecentDateFormatStr(String) recentDateFormatStr} property has
     * been set. In that case, if this property is set true, then the parser,
     * when it encounters a listing parseable with the recent date format, will
     * only consider a date to belong to the previous year if it is more than
     * one day in the future. This will allow all out-of-synch situations
     * (whether based on "slop" - i.e. servers simply out of synch with one
     * another or because of time zone differences - but in the latter case it
     * is highly recommended to use the {@link #setServerTimeZoneId(String)
     * serverTimeZoneId} property instead) to resolve correctly.
     * </p>
     * <p>
     * This is used primarily in unix-based systems.
     * </p>
    ...
    /**
     * <p>
     * setter for the serverTimeZoneId property. This property allows a time
     * zone to be specified corresponding to that known to be used by an FTP
     * server in file listings. This might be particularly useful to clients
     * such as Ant that try to use these timestamps for dependency checking.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * setter for the shortMonthNames property. This property allows the user to
     * specify a set of month names used by the server that is different from
     * those that may be specified using the
     * {@link #setServerLanguageCode(String) serverLanguageCode} property.
     * </p>
     * <p>
     * This should be a string containing twelve strings each composed of three
     * characters, delimited by pipe (|) characters. Currently, only 8-bit ASCII
     * characters are known to be supported. For example, a set of month names
     * used by a hypothetical Icelandic FTP server might conceivably be
     * specified as
    ...
    /**
     * <p>
     * setter for the serverLanguageCode property. This property allows user to
     * specify a <a
     ...
     * set of month names used by the file timestamp parser. If neither this nor
     * the {@link #setShortMonthNames(String) shortMonthNames} is specified,
     * parsing will assume English month names, which may or may not be
     * significant, depending on whether the date format(s) specified via
     * {@link #setDefaultDateFormatStr(String) defaultDateFormatStr} and/or
     * {@link #setRecentDateFormatStr(String) recentDateFormatStr} are using
     * numeric or alphabetic month names.
     * </p>
     * <p>
     * If the code supplied is not supported here, <code>en_US</code> month
     * names will be used. We are supporting here those language codes which,
     * when a <code> java.util.Locale</code> is constucted using it, and a
     * <code>java.text.SimpleDateFormat</code> is constructed using that Locale,
     * the array returned by the SimpleDateFormat's
     * <code>getShortMonths()</code> method consists solely of three 8-bit ASCII
     * character strings. Additionally, languages which do not meet this
     * requirement are included if a common alternative set of short month names
     * is known to be used. This means that users who can tell us of additional
     * such encodings may get them added to the list of supported languages by
     * contacting the Apache Commons Net team.
     * </p>
     * <p>
     * <strong> Please note that this attribute will NOT be used to determine a
     * locale-based date format for the language. </strong> Experience has shown
     * that many if not most FTP servers outside the United States employ the
     * standard <code>en_US</code> date format orderings of
     * <code>MMM d yyyy</code> and <code>MMM d HH:mm</code> and attempting to
     * deduce this automatically here would cause more problems than it would
     * solve. The date format must be changed via the
     * {@link #setDefaultDateFormatStr(String) defaultDateFormatStr} and/or
     ...
     * language codes. Returns a DateFormatSymbols object configured with short
     * month names corresponding to the code. If there is no corresponding entry
     * in the table, the object returned will be that for <code>Locale.US</code>
     * 
     * @param languageCode
     *            See {@link #setServerLanguageCode(String) serverLanguageCode}
     ...
     *         <code>Locale.US</code> if there is no corresponding entry in the
     *         internal table.
     */
    ...
    /**
     * Returns a DateFormatSymbols object configured with short month names as
    ...
    /**
     * Returns a Collection of all the language codes currently supported by
     * this class. See {@link #setServerLanguageCode(String) serverLanguageCode}

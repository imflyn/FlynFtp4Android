...
public class Threader
{
    ...
    /**
     * The client passes in a list of Iterable objects, and the Threader
     * constructs a connected 'graph' of messages
     * 
     * @param messages
     *            iterable of messages to thread
     * @return null if messages == null or root.child == null
     * @since 3.0
     */
    ...
    private void buildContainer(Threadable threadable)
    {
        ...
        // Iterate through all of the references and create ThreadContainers for
        // any references that
        // don't have them.
    ...
    private void pruneEmptyContainers(ThreadContainer parent)
    {
        ...
        for (prev = null, container = parent.child, next = container.next; container != null; prev = container, container = next, next = (container == null ? null : container.next))
        {
            ...
            else if (container.threadable == null && container.child != null && (container.parent != null || container.child.next == null))
            {
                ...
                // promote the children. Make the last child's next point to
                // this container's next
                ...
                // next currently points to the item after the inserted items in
                // the chain - reset that so we process the newly
    ...
    private void gatherSubjects()
    {
        ...
        for (ThreadContainer c = root.child; c != null; c = c.next)
        {
            ...
            // - or this is a dummy container and the old one is not - the dummy
            // one is
        ...
        for (prev = null, c = root.child, rest = c.next; c != null; prev = c, c = rest, rest = (rest == null ? null : rest.next))
        {
            ...
            // We have now found another container in the root set with the same
            // subject
            ...
            {
                // else make the old and new messages be children of a new dummy
                // container.

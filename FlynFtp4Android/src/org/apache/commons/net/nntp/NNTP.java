...
/***
 * The NNTP class is not meant to be used by itself and is provided only so that
 * you may easily implement your own NNTP client if you so desire. If you have
 * no need to perform your own implementation, you should use
 * {@link org.apache.commons.net.nntp.NNTPClient}. The NNTP class is made public
 * to provide access to various NNTP constants and to make it easier for
 * adventurous programmers (or those with special needs) to interact with the
 * NNTP protocol and implement their own clients. A set of methods with names
 * corresponding to the NNTP command names are provided to facilitate this
 * interaction.
 * <p>
 ...
 * properly clean up the system resources used by NNTP. Before disconnecting,
...
public class NNTP extends SocketClient
{
    ...
    /**
     * Wraps {@link SocketClient#_input_} to communicate with server.
     * Initialized by {@link #_connectAction_}. All server reads should be done
     * through this variable.
     */
    protected BufferedReader         _reader_;

    /**
     * Wraps {@link SocketClient#_output_} to communicate with server.
     * Initialized by {@link #_connectAction_}. All server reads should be done
     * through this variable.
     */
    ...
    /***
     * Initiates control connections and gets initial reply, determining if the
     * client is allowed to post to the server. Initializes {@link #_reader_}
    ...
    /***
     * Closes the connection to the NNTP server and sets to null some internal
     * data so that the memory may be reclaimed by the garbage collector. The
     * reply text and code information from the last command is voided so that
     * the memory it used may be reclaimed.
     * <p>
     * 
    ...
    /***
     * Indicates whether or not the client is allowed to post articles to the
     ...
     * 
     * @param command
     *            The text representation of the NNTP command to send.
     * @param args
     *            The arguments to the NNTP command. If this parameter is set to
     ...
     * 
     * @param command
     *            The NNTPCommand constant corresponding to the NNTP command to
     *            send.
     * @param args
     *            The arguments to the NNTP command. If this parameter is set to
     *            null, then the command is sent with no argument.
    ...
    /***
     * Sends an NNTP command with no arguments to the server, waits for a reply
    ...
    /***
     * Returns the integer value of the reply code of the last NNTP reply. You
     * will usually only use this method after you connect to the NNTP server to
     ...
     * {@link #getReplyString getReplyString }. Only use this method if you are
     * implementing your own NNTP client or if you need to fetch a secondary
     * response from the NNTP server.
     * <p>
     * 
     * @return The integer value of the reply code of the fetched NNTP reply. in
    ...
    /***
     * Returns the entire text of the last NNTP server response exactly as it
     * was received, not including the end of line marker.
     * <p>
     * 
     ...
     *            encapsulating &lt and &gt characters.
     ...
     * 
     * @param articleNumber
     *            The number of the article to request from the currently
     *            selected newsgroup.
     * @return The reply code received from the server.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
    ...
    /***
     * A convenience method to send the NNTP HEAD command to the server, receive
     * the initial reply, and return the reply code.
     * <p>
     * 
     * @param messageId
    ...
    /***
     * A convenience method to send the NNTP STAT command to the server, receive
     * the initial reply, and return the reply code.
     * <p>
     * 
     * @param messageId
     ...
     * 
     * @param newsgroup
     *            The name of the newsgroup to select.
     * @return The reply code received from the server.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
    ...
    /***
     * A convenience method to send the NNTP HELP command to the server, receive
     ...
     * 
     * @param messageId
     *            The article identifier, including the encapsulating &lt and
     *            &gt characters.
     * @return The reply code received from the server.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
    ...
    /***
     * A convenience method to send the NNTP LAST command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the NNTP LIST command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the NNTP NEXT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
     ...
     * 
     * @param date
     *            The date after which to check for new groups. Date format is
     *            YYMMDD
     * @param time
     *            The time after which to check for new groups. Time format is
     *            HHMMSS using a 24-hour clock.
     * @param GMT
     *            True if the time is in GMT, false if local server time.
     * @param distributions
     *            Comma-separated distribution list to check for new groups. Set
     *            to null if no distributions.
     * @return The reply code received from the server.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
    ...
    /***
     * A convenience method to send the "NEWNEWS" command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param newsgroups
     *            A comma-separated list of newsgroups to check for new news.
     * @param date
     *            The date after which to check for new news. Date format is
     *            YYMMDD
     * @param time
     *            The time after which to check for new news. Time format is
     *            HHMMSS using a 24-hour clock.
     * @param GMT
     *            True if the time is in GMT, false if local server time.
     * @param distributions
     *            Comma-separated distribution list to check for new news. Set
     *            to null if no distributions.
     * @return The reply code received from the server.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
    ...
    /***
     * A convenience method to send the NNTP POST command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
    ...
    /***
     * A convenience method to send the NNTP QUIT command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @return The reply code received from the server.
     ...
     * @param username
     *            A valid username.
     * @return The reply code received from the server. The server should return
     *         a 381 or 281 for this command.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
     *                causing the server to send NNTP reply code 400. This
     ...
     * receive the reply, and return the reply code. If this step is required,
     * it should immediately follow the AUTHINFO USER command (See RFC 2980)
     * <p>
     * 
     * @param password
     *            a valid password.
     * @return The reply code received from the server. The server should return
     *         a 281 or 502 for this command.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
     *                causing the server to send NNTP reply code 400. This
     ...
     *            a String representation of the range of article headers
     *            required. This may be an article number, or a range of article
     *            numbers in the form "XXXX-YYYY", where XXXX and YYYY are valid
     *            article numbers in the current group. It also may be of the
     *            form "XXX-", meaning "return XXX and all following articles"
    ...
    /***
     * A convenience method to send the NNTP XHDR command to the server, receive
     * the reply, and return the reply code.
     * <p>
     * 
     * @param header
     *            a String naming a header line (e.g., "subject"). See RFC-1036
     *            for a list of valid header lines.
     * @param selectedArticles
     *            a String representation of the range of article headers
     *            required. This may be an article number, or a range of article
     *            numbers in the form "XXXX-YYYY", where XXXX and YYYY are valid
     ...
     * 
     * @param wildmat
     *            A wildmat (pseudo-regex) pattern. See RFC 2980 for details.
     * @return the reply code received from the server.
     * @throws IOException
     */

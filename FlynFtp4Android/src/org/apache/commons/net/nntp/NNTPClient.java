...
/***
 * NNTPClient encapsulates all the functionality necessary to post and retrieve
 * articles from an NNTP server. As with all classes derived from
 ...
 * {@link org.apache.commons.net.nntp.NNTP#disconnect disconnect() } after
 * you're completely finished interacting with the server. Remember that the
 * {@link org.apache.commons.net.nntp.NNTP#isAllowedToPost isAllowedToPost()}
 * method is defined in {@link org.apache.commons.net.nntp.NNTP}.
 * <p>
 * You should keep in mind that the NNTP server may choose to prematurely close
 * a connection if the client has been idle for longer than a given time period
 * or if the server is being shutdown by the operator or some other reason. The
 * NNTP class will detect a premature NNTP server connection closing when it
 * receives a {@link org.apache.commons.net.nntp.NNTPReply#SERVICE_DISCONTINUED
 * NNTPReply.SERVICE_DISCONTINUED } response to a command. When that occurs, the
 * NNTP class method encountering that reply will throw an
 * {@link org.apache.commons.net.nntp.NNTPConnectionClosedException} .
 * <code>NNTPConectionClosedException</code> is a subclass of
 ...
 * an {@link org.apache.commons.net.nntp.NNTPConnectionClosedException} , you
 * must disconnect the connection with
 * {@link org.apache.commons.net.nntp.NNTP#disconnect disconnect() } to properly
 * clean up the system resources used by NNTP. Before disconnecting, you may
 * check the last reply code and text with
...
public class NNTPClient extends NNTP
{
     ...
     * 
     * @param reply
     *            the reply to parse "22n nnn <aaa>"
     * @param pointer
    ...
    /*
     * 211 n f l s group selected (n = estimated number of articles in group, f
     * = first article number in the group, l = last article number in the
     * group, s = name of the group.)
     */
     ...
     * @param line
     *            a response line
     * @return the parsed {@link Article}, if unparseable then isDummy() will be
    ...
    /***
     * Retrieves an article from the NNTP server. The article is referenced by
     * its unique article identifier (including the enclosing &lt and &gt). The
     * article number and identifier contained in the server reply are returned
     * through an ArticleInfo. The <code> articleId </code> field of the
     * ArticleInfo cannot always be trusted because some NNTP servers do not
     * correctly follow the RFC 977 reply format.
     * <p>
     ...
     * issuing commands as it does for returning results. Therefore the returned
     * BufferedReader actually reads directly from the NNTP connection. After
     * the end of message has been reached, new commands can be executed and
     * their replies read. If you do not follow these requirements, your program
     * will not work properly.
     * <p>
     * 
     * @param articleId
     *            The unique article identifier of the article to retrieve. If
     *            this parameter is null, the currently selected article is
     *            retrieved.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     *            because of server deviations from RFC 977 reply formats. You
     *            may set this parameter to null if you do not desire to
     *            retrieve the returned article information.
     * @return A DotTerminatedMessageReader instance from which the article be
     *         read. null if the article does not exist.
    ...
    /**
     * Same as <code> retrieveArticle((String) null) </code> Note: the return
    ...
    /***
     * Retrieves an article from the currently selected newsgroup. The article
     * is referenced by its article number. The article number and identifier
     * contained in the server reply are returned through an ArticleInfo. The
     * <code> articleId </code> field of the ArticleInfo cannot always be
     * trusted because some NNTP servers do not correctly follow the RFC 977
     * reply format.
     * <p>
     * A DotTerminatedMessageReader is returned from which the article can be
     * read. If the article does not exist, null is returned.
     ...
     * 
     * @param articleNumber
     *            The number of the the article to retrieve.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     *            because of server deviations from RFC 977 reply formats. You
     ...
     * referenced by its unique article identifier (including the enclosing &lt
     * and &gt). The article number and identifier contained in the server reply
     * are returned through an ArticleInfo. The <code> articleId </code> field
     * of the ArticleInfo cannot always be trusted because some NNTP servers do
     * not correctly follow the RFC 977 reply format.
     * <p>
     * A DotTerminatedMessageReader is returned from which the article can be
     * read. If the article does not exist, null is returned.
     * <p>
     ...
     *            being retrieved. If this parameter is null, the header of the
     *            currently selected article is retrieved.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     *            because of server deviations from RFC 977 reply formats. You
    ...
    /**
     * Same as
     * <code> retrieveArticleHeader(articleId, (ArticleInfo) null) </code> Note:
     * the return can be cast to a {@link BufferedReader}
     */
    ...
    /**
     * Same as <code> retrieveArticleHeader((String) null) </code> Note: the
     * return can be cast to a {@link BufferedReader}
     */
     ...
     * article is referenced by its article number. The article number and
     * identifier contained in the server reply are returned through an
     * ArticleInfo. The <code> articleId </code> field of the ArticleInfo cannot
     * always be trusted because some NNTP servers do not correctly follow the
     * RFC 977 reply format.
     * <p>
     * A DotTerminatedMessageReader is returned from which the article can be
     * read. If the article does not exist, null is returned.
     * <p>
     ...
     * 
     * @param articleNumber
     *            The number of the the article whose header is being retrieved.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     *            because of server deviations from RFC 977 reply formats. You
    ...
    /***
     * Retrieves an article body from the NNTP server. The article is referenced
     * by its unique article identifier (including the enclosing &lt and &gt).
     * The article number and identifier contained in the server reply are
     * returned through an ArticleInfo. The <code> articleId </code> field of
     * the ArticleInfo cannot always be trusted because some NNTP servers do not
     * correctly follow the RFC 977 reply format.
     * <p>
     * A DotTerminatedMessageReader is returned from which the article can be
     * read. If the article does not exist, null is returned.
     ...
     * 
     * @param articleId
     *            The unique article identifier of the article whose body is
     *            being retrieved. If this parameter is null, the body of the
     *            currently selected article is retrieved.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     ...
     *            may set this parameter to null if you do not desire to
     *            retrieve the returned article information.
     * @return A DotTerminatedMessageReader instance from which the article body
    ...
    /**
     * Same as <code> retrieveArticleBody(null) </code> Note: the return can be
     ...
     * 
     * @param articleNumber
     *            The number of the the article whose body is being retrieved.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
     *            because of server deviations from RFC 977 reply formats. You
     ...
     * contained in the server reply through the info parameter.
     * <p>
     * 
     * @param newsgroup
     *            The newsgroup to select.
     * @param info
     *            A parameter through which the newsgroup information of the
     *            selected newsgroup contained in the server reply is returned.
    ...
    /***
     * Select an article by its unique identifier (including enclosing &lt and
     * &gt) and return its article number and id through the pointer parameter.
     * This is achieved through the STAT command. According to RFC 977, this
     * will NOT set the current article pointer on the server. To do that, you
     * must reference the article by its number.
     * <p>
     * 
     * @param articleId
     *            The unique article identifier of the article that is being
     *            selectedd. If this parameter is null, the body of the current
     *            article is selected
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. The articleId field cannot always be trusted
    ...
    /***
     * Select an article in the currently selected newsgroup by its number. and
     * return its article number and id through the pointer parameter. This is
     * achieved through the STAT command. According to RFC 977, this WILL set
     * the current article pointer on the server. Use this command to select an
     * article before retrieving it, or to obtain an article's unique identifier
     * given its number.
     * <p>
     * 
     * @param articleNumber
     *            The number of the article to select from the currently
     *            selected newsgroup.
     * @param pointer
     *            A parameter through which to return the article's number and
     *            unique id. Although the articleId field cannot always be
     *            trusted because of server deviations from RFC 977 reply
     *            formats, we haven't found a server that misformats this
     *            information in response to this particular command. You may
     *            set this parameter to null if you do not desire to retrieve
     ...
     * currently selected newsgroup and return its number and unique id through
     * the pointer parameter. Because of deviating server implementations, the
     * articleId information cannot be trusted. To obtain the article
     * identifier, issue a
     ...
     * served, a zero length array will be returned. If the command fails, null
     * will be returned. The method uses the "LIST" command.
     * <p>
     * 
     ...
     *         newsgroup served by the NNTP server. If no newsgroups are served,
     ...
     *         array will be returned. If the command fails, null will be
     *         returned.
     * @throws IOException
     ...
     *            a pseudo-regex pattern (cf. RFC 2980)
     * @return An iterable of Strings containing the raw information for each
     *         newsgroup served by the NNTP server corresponding to the supplied
     *         pattern. If no such newsgroups are served, no entries will be
     *         returned.
     * @throws IOException
     * @since 3.0
     */
    ...
    /***
     * List all new newsgroups added to the NNTP server since a particular date
     * subject to the conditions of the specified query. If no new newsgroups
     * were added, a zero length array will be returned. If the command fails,
     * null will be returned. This uses the "NEWGROUPS" command.
     * <p>
     * 
     * @param query
     ...
     * List all new newsgroups added to the NNTP server since a particular date
     * subject to the conditions of the specified query. If no new newsgroups
     * were added, no entries will be returned. This uses the "NEWGROUPS"
     * command.
     * <p>
     * 
     * @param query
     *            The query restricting how to search for new newsgroups.
     * @return An iterable of Strings containing the raw information for each
     *         new newsgroup added to the NNTP server. If no newsgroups were
     *         added, no entries will be returned.
     * @exception NNTPConnectionClosedException
     *                If the NNTP server prematurely closes the connection as a
     *                result of the client being idle or some other reason
     *                causing the server to send NNTP reply code 400. This
     ...
     * subject to the conditions of the specified query. If no new new news is
     * found, a zero length array will be returned. If the command fails, null
     * will be returned. You must add at least one newsgroup to the query, else
     * the command will fail. Each String in the returned array is a unique
     * message identifier including the enclosing &lt and &gt. This uses the
     * "NEWNEWS" command.
     * <p>
     * 
     * @param query
     *            The query restricting how to search for new news. You must add
     *            at least one newsgroup to the query.
     ...
     * List all new articles added to the NNTP server since a particular date
     * subject to the conditions of the specified query. If no new new news is
     * found, no entries will be returned. This uses the "NEWNEWS" command. You
     * must add at least one newsgroup to the query, else the command will fail.
     * Each String which is returned is a unique message identifier including
     * the enclosing &lt and &gt.
     * <p>
     * 
     * @param query
     *            The query restricting how to search for new news. You must add
    ...
    /***
     * There are a few NNTPClient methods that do not complete the entire
     * sequence of NNTP commands to complete a transaction. These commands
     * require some action by the programmer after the reception of a positive
     * preliminary command. After the programmer's code completes its actions,
     ...
     * DotTerminatedMessageWriter instance to which the article can be written.
     * Null is returned if the posting attempt fails. You should check
     * {@link NNTP#isAllowedToPost isAllowedToPost() } before trying to post.
     * However, a posting attempt can fail due to malformed headers.
     * <p>
     * You must not issue any commands to the NNTP server (i.e., call any (other
     * methods) until you finish writing to the returned Writer instance and
     * close it. The NNTP protocol uses the same stream for issuing commands as
     * it does for returning results. Therefore the returned Writer actually
     * writes directly to the NNTP connection. After you close the writer, you
     * can execute new commands. If you do not follow these requirements your
     * program will not work properly.
     * <p>
     * Different NNTP servers will require different header formats, but you can
     * use the provided {@link org.apache.commons.net.nntp.SimpleNNTPHeader}
     * class to construct the bare minimum acceptable header for most news
     * readers. To construct more complicated headers you should refer to RFC
     * 822. When the Java Mail API is finalized, you will be able to use it to
     * compose fully compliant Internet text messages. The
     * DotTerminatedMessageWriter takes care of doubling line-leading dots and
     * ending the message with a single dot upon closing, so all you have to
     * worry about is writing the header and the message.
     * <p>
     * Upon closing the returned Writer, you need to call
     * {@link #completePendingCommand completePendingCommand() } to finalize the
     * posting and verify its success or failure from the server reply.
     * <p>
     * 
     ...
     * However, you must still disconnect from the server before you can open a
     * new connection.
     * <p>
     * 
     * @return True if successfully completed, false if not.
    ...
    /**
     * Log into a news server by sending the AUTHINFO USER/AUTHINFO PASS command
     * sequence. This is usually sent in response to a 480 reply code from the
     * NNTP server.
     * <p>
     * 
     * @param username
     ...
     * 
     * See {@link NNTP#xover} for legal agument formats. Alternatively, read RFC
     * 2980 :-)
     * <p>
     * 
     * @param articleRange
     ...
     * 
     * See {@link NNTP#xhdr} for legal agument formats. Alternatively, read RFC
     * 1036.
     * <p>
     * 
     * @param header
    ...
    /**
     * Return an article header for all articles between lowArticleNumber and
     * highArticleNumber, inclusively.
     * <p>
     * 
     * @param header
     * @param lowArticleNumber

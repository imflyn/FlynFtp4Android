...
final class TelnetInputStream extends BufferedInputStream implements Runnable
{
     ...
     * 
     * @param mayBlock
     *            true if method is allowed to block
     * @return the next byte of data, or -1 (EOF) if end of stread reached, or
     *         -2 (WOULD_BLOCK) if mayBlock is false and there is no data
     *         available
     */
    private int __read(boolean mayBlock) throws IOException
    {
        ...
        while (true)
        {
            ...
            // return WOULD_BLOCK (-2). (More efficient than exception.)
            ...
            switch (__receiveState)
            {
                ...
                case _STATE_CR:
                    ...
                    // Handle as normal data by falling through to _STATE_DATA
                    // case
                ...
                case _STATE_IAC:
                    switch (ch)
                    {
                        ...
                        case TelnetCommand.IAC:
                            __receiveState = _STATE_DATA;
                            break; // exit to enclosing switch to return IAC
                                   // from read
                        case TelnetCommand.SE: // unexpected byte! ignore it
                                               // (don't send it as a command)
                            __receiveState = _STATE_DATA;
                            continue;
                        default:
                            __receiveState = _STATE_DATA;
                ...
                if (__bytesAvailable == 0)
                {
                    ...
                    } else
                    {
                        ...
                        do
                        {
                            ...
                            // Potentially, this could happen if the
                            // remaining buffered socket data consists entirely
                            // of Telnet command sequence and no "user" data.
                            mayBlock = false;

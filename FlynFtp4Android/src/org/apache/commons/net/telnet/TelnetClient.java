...
/***
 * The TelnetClient class implements the simple network virtual terminal (NVT)
 * for the Telnet protocol according to RFC 854. It does not implement any of
 * the extra Telnet options because it is meant to be used within a Java program
 ...
 * {@link org.apache.commons.net.SocketClient#connect connect} method. Then an
 * InputStream and OutputStream for sending and receiving data over the Telnet
 * connection can be obtained by using the {@link #getInputStream
 * getInputStream() } and {@link #getOutputStream getOutputStream() } methods.
 * When you finish using the streams, you must call {@link #disconnect
 * disconnect } rather than simply closing the streams.
 * <p>
 * <p>
 * 
...
public class TelnetClient extends Telnet
{
     ...
     * 
     * @param termtype
     *            the terminal type to use, e.g. {@code VT100}
     */
    /* TERMINAL-TYPE option (start) */
     ...
     * well as the socket. If you have references to the input and output
     * streams of the telnet connection, you should not close them yourself, but
     * rather call disconnect to properly close the connection.
     ***/
    @Override
     ...
     * {@link TelnetClient} will add the IAC SB &amp; IAC SE framing bytes; the
     * first byte in {@code message} should be the appropriate telnet option
     * code.
     * 
     * <p>
     * This method does not wait for any response. Subnegotiation messages sent
     ...
     * @throws IllegalArgumentException
     *             if {@code message} has length zero
     * @throws IOException
     ...
     * 
     * <p>
     * This method does not wait for any response. Messages sent by the remote
     * end can be handled by registering an approrpriate
     * {@link TelnetOptionHandler}.
     * </p>
     * 
     * @param command
    ...
    /***
     * Registers an OutputStream for spying what's going on in the TelnetClient
     * session.
     * <p>
     * 
     * @param spystream
     ...
     * immediate handling of option negotiation, notifications, etc. (at least
     * until the fixed-size internal buffer fills up). Otherwise, no thread is
     * created an all negotiation and option handling is deferred until a read()
     * is performed on the {@link #getInputStream input stream}.
     * </p>
     * 
     * <p>
     ...
     * 
     * @param flag
     *            true to enable the reader thread, false to disable
    ...
    /***
     * Register a listener to be notified when new incoming data is available to
     * be read on the {@link #getInputStream input stream}. Only one listener is
     * supported at a time.
     * 
     * <p>
     * More precisely, notifications are issued whenever the number of bytes
     * available for immediate reading (i.e., the value returned by
     * {@link InputStream#available}) transitions from zero to non-zero. Note
     * that (in general) multiple reads may be required to empty the buffer and
     * reset this notification, because incoming bytes are being added to the
     * internal buffer asynchronously.
     * </p>
     * 
     * <p>
     * Notifications are only supported when a {@link #setReaderThread reader
     * thread} is enabled for the connection.
     * </p>
     * 
     * @param listener
     *            listener to be registered; replaces any previous
     * @since 3.0
     ***/

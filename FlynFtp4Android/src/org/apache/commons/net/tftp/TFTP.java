 ...
 * protocol directly, in case you want to write your own TFTP client or server.
 * However, almost every user should only be concerend with the
 * {@link org.apache.commons.net.DatagramSocketClient#open open() }, and
 * {@link org.apache.commons.net.DatagramSocketClient#close close() }, methods.
 * Additionally,the a
...
public class TFTP extends DatagramSocketClient
{
    ...
    /***
     * The default number of milliseconds to wait to receive a datagram before
     * timing out. The default is 5000 milliseconds (5 seconds).
     ***/
    ...
    /***
     * A buffer used to accelerate sends in bufferedSend(). It is left package
     * visible so that TFTPClient may be slightly more efficient during file
     * sends. It saves the creation of an additional buffer and prevents a
     * buffer copy in _newDataPcket().
     ***/
    byte[]                  _sendBuffer;

    /***
     * Returns the TFTP string representation of a TFTP transfer mode. Will
     * throw an ArrayIndexOutOfBoundsException if an invalid transfer mode is
     * specified.
     * <p>
     * 
     * @param mode
    ...
    /***
     * Creates a TFTP instance with a default timeout of DEFAULT_TIMEOUT, a null
     * socket, and buffered operations disabled.
     ***/
    public TFTP()
    {
    ...
    /***
     * This method synchronizes a connection by discarding all packets that may
     * be in the local socket buffer. This method need only be called when you
    ...
    /***
     * This is a special method to perform a more efficient packet receive. It
     * should only be used after calling {@link #beginBufferedOps
     * beginBufferedOps() }. beginBufferedOps() initializes a set of buffers
     * used internally that prevent the new allocation of a DatagramPacket and
     * byte array for each send and receive. To use these buffers you must call
     * the bufferedReceive() and bufferedSend() methods instead of send() and
     * receive(). You must also be certain that you don't manipulate the
     * resulting packet in such a way that it interferes with future buffered
     * operations. For example, a TFTPDataPacket received with bufferedReceive()
     * will have a reference to the internal byte buffer. You must finish using
     * this data before calling bufferedReceive() again, or else the data will
     * be overwritten by the the call.
     * <p>
     * 
     * @return The TFTPPacket received.
     * @exception InterruptedIOException
     *                If a socket timeout occurs. The Java documentation claims
     *                an InterruptedIOException is thrown on a DatagramSocket
     *                timeout, but in practice we find a SocketException is
     *                thrown. You should catch both to be safe.
    ...
    /***
     * This is a special method to perform a more efficient packet send. It
     * should only be used after calling {@link #beginBufferedOps
     * beginBufferedOps() }. beginBufferedOps() initializes a set of buffers
     * used internally that prevent the new allocation of a DatagramPacket and
     * byte array for each send and receive. To use these buffers you must call
     ...
     * {@link #bufferedSend bufferedSend() } and {@link #bufferedReceive
     * bufferedReceive() }. This method must be called before calling either one
     * of those two methods. When you finish using buffered operations, you must

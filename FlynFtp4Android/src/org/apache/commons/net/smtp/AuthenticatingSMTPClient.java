...
{
    /**
     * The default AuthenticatingSMTPClient constructor. Creates a new
     ...
     *            the protocol.
     * @param implicit
     *            The security mode, {@code true} for implicit, {@code false}
     *            for explicit
     * @since 3.3
     */
    ...
    /***
     * Returns the integer values of the enhanced reply code of the last SMTP
     * reply.
     * 
     * @return The integer values of the enhanced reply code of the last SMTP
     *         reply. First digit is in the first array element.
     ***/
     ...
     * 
     * @param method
     *            the method to use, one of the
     *            {@link AuthenticatingSMTPClient.AUTH_METHOD} enum values
     * @param username
     *            the user name. If the method is XOAUTH, then this is used as
     *            the plain text oauth protocol parameter string which is
     *            Base64-encoded for transmission.
     * @param password
     *            the password for the username. Ignored for XOAUTH.
     * 
     * @return True if successfully completed, false if not.
     * @exception SMTPConnectionClosedException
     *                If the SMTP server prematurely closes the connection as a
     ...
     *                If the CRAM hash algorithm cannot be instantiated by the
     *                Java runtime system.
     * @exception InvalidKeyException
     *                If the CRAM hash algorithm failed to use the given
     *                password.
    ...
    public boolean auth(AuthenticatingSMTPClient.AUTH_METHOD method, String username, String password) throws IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException
    {
        ...
        if (method.equals(AUTH_METHOD.PLAIN))
        {
            ...
            // read it.
            return SMTPReply.isPositiveCompletion(sendCommand(Base64.encodeBase64StringUnChunked(("\000" + username + "\000" + password).getBytes(getCharsetName())) // Java
                                                                                                                                                                     // 1.6
                                                                                                                                                                     // can
                                                                                                                                                                     // use
                                                                                                                                                                     // getCharset()
        ...
        } else if (method.equals(AUTH_METHOD.CRAM_MD5))
        {
            ...
            hmac_md5.init(new SecretKeySpec(password.getBytes(getCharsetName()), "HmacMD5")); // Java
                                                                                              ...
                                                                                              // getCharset()
            // compute the result:
            byte[] hmacResult = _convertToHexString(hmac_md5.doFinal(serverChallenge)).getBytes(getCharsetName()); // Java
                                                                                                                   // 1.6
                                                                                                                   // can
                                                                                                                   // use
                                                                                                                   // getCharset()
            // join the byte arrays to form the reply
            byte[] usernameBytes = username.getBytes(getCharsetName()); // Java
                                                                        // 1.6
                                                                        // can
                                                                        // use
                                                                        // getCharset()
        ...
        } else if (method.equals(AUTH_METHOD.LOGIN))
        {
            ...
            return SMTPReply.isPositiveCompletion(sendCommand(Base64.encodeBase64StringUnChunked(password.getBytes(getCharsetName())))); // Java
                                                                                                                                         // 1.6
                                                                                                                                         // can
                                                                                                                                         // use
                                                                                                                                         // getCharset()
        ...
        {
            return SMTPReply.isPositiveIntermediate(sendCommand(Base64.encodeBase64StringUnChunked(username.getBytes(getCharsetName())) // Java
                                                                                                                                        // 1.6
                                                                                                                                        // can
                                                                                                                                        // use
                                                                                                                                        // getCharset()
    ...
    /**
     * Converts the given byte array to a String containing the hex values of
     * the bytes. For example, the byte 'A' will be converted to '41', because
    ...
    {
        /**
         * The standarised (RFC4616) PLAIN method, which sends the password
         * unencrypted (insecure).
         */
        PLAIN,
        /**
         * The standarised (RFC2195) CRAM-MD5 method, which doesn't send the
         * password (secure).
         */
        CRAM_MD5,
        /**
         ...
         * 
         * @param method
         *            The authentication method to get the name for.
         * @return The name of the given authentication method suitable for the
         *         server.
         */

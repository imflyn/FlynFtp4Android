...
/**
 * SMTP over SSL processing. Copied from FTPSClient.java and modified to suit
 * SMTP. If implicit mode is selected (NOT the default), SSL/TLS negotiation
 * starts right after the connection has been established. In explicit mode (the
 * default), SSL/TLS negotiation starts when the user calls execTLS() and the
 * server accepts the command. Implicit usage: SMTPSClient c = new
 * SMTPSClient(true); c.connect("127.0.0.1", 465); Explicit usage: SMTPSClient c
 * = new SMTPSClient(); c.connect("127.0.0.1", 25); if (c.execTLS()) { /rest of
 * the commands here/ }
 * 
 * @since 3.0
 */
public class SMTPSClient extends SMTPClient
{
    ...
    /**
     * The {@link TrustManager} implementation, default null (i.e. use system
     * managers).
     */
    private TrustManager        trustManager     = null;

    /** The {@link KeyManager}, default null (i.e. use system managers). */
    private KeyManager          keyManager       = null; // seems not to be
                                                          // required
     ...
     * 
     * @throws IOException
     *             If it is thrown by _connectAction_().
    ...
    /**
     * SSL/TLS negotiation. Acquires an SSL socket of a connection and carries
     * out handshake processing.
     * 
     * @throws IOException
     ...
     * this connection. When the underlying {@link java.net.Socket Socket} is
     ...
     * for use on this connection. When the underlying {@link java.net.Socket
     * Socket} is not an {@link SSLSocket} instance, returns null.
     * 
     ...
     * 
     * @throws IOException
     *             If an I/O error occurs while sending the command or
